...
app.put("/find-one-update/:genre", async (req, res) => {
  try {
    // Find the first instance of a document that contains a name with the value equal to 'Kids'
    const existingGenre = await Genre.findOne({ name: "Kids" });
    ...
    // Update the name with the value given from the URL param
    existingGenre.name = req.params.genre;
