...
export default function ProfilePage() {
  ...
  const fetchData = async () => {
    try {
      const { data } = await API.getSingleUser(id); // Use the 'id' obtained from useParams
      setUser(data);
    } catch (error) {
  ...
  useEffect(() => {
    fetchData();
  }, [id]); // Add 'id' to the dependency array to re-fetch data when the 'id' parameter changes
